...
export function MachineDetailsPage() {
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full border border-stone-200 text-center space-y-4">
          ...
          <h2 className="text-xl font-bold text-stone-900">Machine Not Found</h2>
          <p className="text-sm text-stone-600">
            The requested agricultural machinery listing might have been removed or deactivated.
          </p>
          <Link
            to="/find-machinery"
  ...
  const handleToggleFav = () => {
    ...
    setFavourited(next);
    showToast(
      next ? 'Saved to Favourites' : 'Removed from Favourites',
  ...
  const handleToggleCompare = () => {
    if (inCompare) {
      ...
      showToast('Removed from Compare', `${machine.name} removed.`, 'info');
    } else {
      const ok = addToCompare(machine.id);
      if (ok) {
        showToast('Added to Compare', `${machine.name} added to comparison dock.`, 'success');
      } else {
        showToast('Comparison Limit', 'Maximum 4 machines can be compared simultaneously.', 'error');
      }
    }
  };
    ...
    <div className="bg-stone-50 min-h-screen py-8 text-stone-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between gap-4 mb-6">
          <button
            onClick={() => navigate(-1)}
            className="inline-flex items-center gap-1.5 text-xs font-bold text-stone-600 hover:text-emerald-800 transition-colors p-1"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{favourited ? 'Saved in Favourites' : 'Save Machine'}</span>
            </button>
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white rounded-3xl border border-stone-200 p-6 sm:p-8 shadow-xs space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                >
                  ...
                  {machine.isAvailable ? 'Available for Immediate Booking' : 'Currently Rented'}
                </span>
              </div>

              <h1 className="text-2xl sm:text-3xl font-black text-stone-900">{machine.name}</h1>
              <p className="text-xs font-semibold text-stone-500 -mt-2">Model: {machine.model}</p>
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs text-stone-600 pt-2 border-t border-stone-100">
                <div className="flex items-center gap-1 font-semibold text-stone-800">
                  ...
                  <span>{machine.rating} Rating</span>
                  <span className="text-stone-400">({machine.reviewCount} reviews)</span>
                </div>
                <span>•</span>
                <div className="flex items-center gap-1">
                  <MapPin className="w-4 h-4 text-emerald-700" />
                  <span>{machine.location} ({machine.distanceKm} km away)</span>
                </div>
                <span>•</span>
                <div className="flex items-center gap-1">
            ...
            <div className="bg-white rounded-3xl border border-stone-200 p-6 sm:p-8 shadow-xs space-y-4">
              ...
              <div className="pt-2 text-xs text-stone-600 flex items-center gap-2">
                <span className="font-bold text-stone-800">Ideal Farm Size:</span>
                <span>
                  {machine.suitableFarmSize.minAcres} to {machine.suitableFarmSize.maxAcres} acres
                </span>
              </div>
            </div>
            ...
            {/* Owner Details Card */}
            <div className="bg-white rounded-3xl border border-stone-200 p-6 sm:p-8 shadow-xs space-y-4">
              <h3 className="text-lg font-black text-stone-900">Machinery Owner Information</h3>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-stone-500 flex items-center gap-1.5">
                      ...
                      <strong>{machine.ownerRating}</strong> (Verified Owner)
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-3xl border border-stone-200 p-6 sm:p-8 shadow-xs space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-black text-stone-900">Farmer Reviews</h3>
                <span className="text-xs font-semibold text-emerald-800">⭐ {machine.rating} Average</span>
              </div>
            ...
            <div className="bg-white rounded-3xl border border-stone-200 p-6 shadow-lg space-y-5 sticky top-24">
              <div className="border-b border-stone-100 pb-4">
                ...
                <div className="text-xs font-bold text-stone-600 mt-0.5">
                  Full Day: ₹{machine.dailyRate.toLocaleString()} / day
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-950 to-emerald-900 text-white space-y-3">
                ...
                <button
                  ...
                  className="w-full py-1.5 text-center text-xs font-bold text-emerald-200 hover:text-white bg-white/10 hover:bg-white/20 rounded-xl transition-colors"
                >
                  View 5-Factor Score Breakdown →
                </button>
              </div>
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="text-[11px] text-stone-500 text-center flex items-center justify-center gap-1 pt-1">
                  ...
                  <span>Zero advance required. Pay on completion.</span>
                </div>
              </div>
            </div>
          </div>
